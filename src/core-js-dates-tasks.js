...
function getNextFriday(date) {
  ...
  if (day < 5) {
    return new Date(date.setDate(date.getDate() + 5 - day));
  }

  return new Date(date.setDate(date.getDate() + 7 - day + 5));
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return 1 + (new Date(dateEnd) - new Date(dateStart)) / 1000 / 60 / 60 / 24;
}
 ...
 */
// ISO dates are sortable, so we can use just a string comparison
